...
function Send-Round {
    param([System.Net.Sockets.NetworkStream]$Stream, [string]$TypeName)
...
try {
    ...
    Send-Round -Stream $stream -TypeName 'UBurstA'
    Send-Round -Stream $stream -TypeName 'UBurstB'
    Send-Round -Stream $stream -TypeName 'UBurstC'
